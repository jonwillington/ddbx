...
/** Generic metric-mode sheet — drives the alpha-vs-raw toggle and the
 *  trade-vs-disclosure anchor. All market-specific copy (benchmark name,
 *  disclosure register) flows in as props so US/SE can opt in via their
 *  own MarketConfig.MetricModeSheet wrapper. */
export function MetricModeSheet({
  open,
  onClose,
  benchmarkName,
  disclosureRegisterLabel,
}: {
  open: boolean;
  onClose: () => void;
  /** Full benchmark name used in the alpha explanation — e.g. "FTSE
   *  All-Share", "S&P 500", "OMXS30". */
  benchmarkName: string;
  /** Name of the regulatory disclosure register — e.g. "RNS" for the UK,
   *  "SEC EDGAR" for the US. Threaded into the disclosure-anchor copy. */
  disclosureRegisterLabel: string;
}) {
  const { comparison, anchor, setComparison, setAnchor } =
    useDashboardMetricMode();

  const comparisonExplanation: Record<DashboardComparison, string> = {
    ...
    market: `Subtracts the ${benchmarkName}'s return over the same window, so the number is alpha. Positive means the stock beat the market, negative means it lagged.`,
  };

  const anchorCopy: Record<
    DashboardAnchor,
    { title: string; description: string }
  > = {
    disclosure: {
      title: "Disclosure date",
      description: `When the trade was published on ${disclosureRegisterLabel} and became visible to everyone. This is the window a copycat investor could actually have acted in, so it's the more honest measure of the signal's value.`,
    },
    trade: {
      title: "Trade date",
      description:
      ...
      >
        ...
        <div className="overflow-y-auto px-4 py-5 space-y-6">
          ...
          <section className="space-y-3">
            <h3 className="text-[15px] font-semibold px-1">
              What do you want to track against?
            </h3>
            <div
          ...
          <section className="space-y-2">
            ...
            <div className="rounded-xl border border-black/[0.06] dark:border-white/[0.06] bg-surface/60 divide-y divide-black/[0.06] dark:divide-white/[0.06]">
              {(["disclosure", "trade"] as const).map((a) => {
                const { title, description } = anchorCopy[a];
